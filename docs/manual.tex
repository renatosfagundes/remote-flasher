...
\textit{Ver exemplo \texttt{05\_can\_basic} na pasta \texttt{examples/}.}

% === 15. Exemplos do Repositório ==================================
...
\begin{tabularx}{\textwidth}{clX}
  ...
  02 & \texttt{multitask} & Múltiplas tasks com prioridades diferentes e preempção \\
  03 & \texttt{alarm\_dynamic} & \texttt{SetRelAlarm()}, \texttt{CancelAlarm()}, mudança de período \\
  04 & \texttt{prodcons} & \texttt{RESOURCE}, \texttt{GetResource()}, \texttt{ReleaseResource()}, produtor-consumidor \\
  05 & \texttt{can\_basic} & TX + RX com MCP2515, \texttt{sendMsgBuf()}, \texttt{readMsgBuf()} \\
  \bottomrule
\end{tabularx}
\end{center}

Cada exemplo contém arquivos \texttt{.oil} e \texttt{.cpp} prontos. Para projetos CAN, copie também \texttt{Board.h} do template.
